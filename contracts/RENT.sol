...
contract RENT {
    ...
    */
    ...
    function counterOffer(
        ...
        require(
            _o.status == Status.Active || _o.oorco == OorCO.O,
            "Offer doesn't exist!"
        );
        require(
            _o.status != Status.Active || _o.oorco == OorCO.CO,
    ...
    function approve(bytes memory _id, bool _sellerOrBuyer) public {
        ...
        if (!_sellerOrBuyer) {
            require(
                d.oorco == OorCO.CO || d.seller == msg.sender,
    ...
    function finaliseDeal(bytes memory _id, address _buyer) public {
        Offer memory _offer = listings[_id];
        // Router pays the price in the callback function
        Callback c = new Callback();
        c.callback(_offer.price, _buyer);
    ...
    function getListingInfo(bytes memory _hashId)
        public
        view
        returns (
            address seller,
